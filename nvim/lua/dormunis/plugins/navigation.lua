...
return {
  ...
  },
  {
    ...
    config = function()
      ...
      end, { desc = "Navigate to previous marked file", silent = true, noremap = true })
      vim.keymap.set("n", "=1", function()
        harpoon:list():select(1)
      end, { desc = "Navigate harpoon ID 1", silent = true, noremap = true })
      vim.keymap.set("n", "=2", function()
        harpoon:list():select(2)
      end, { desc = "Navigate harpoon ID 2", silent = true, noremap = true })
      vim.keymap.set("n", "=3", function()
        harpoon:list():select(3)
      end, { desc = "Navigate harpoon ID 3", silent = true, noremap = true })
      vim.keymap.set("n", "=4", function()
        harpoon:list():select(4)
      end, { desc = "Navigate harpoon ID 4", silent = true, noremap = true })
      vim.keymap.set("n", "=5", function()
        harpoon:list():select(5)
      end, { desc = "Navigate harpoon ID 5", silent = true, noremap = true })
      vim.keymap.set("n", "=6", function()
        harpoon:list():select(6)
      end, { desc = "Navigate harpoon ID 6", silent = true, noremap = true })
      vim.keymap.set("n", "=7", function()
        harpoon:list():select(7)
      end, { desc = "Navigate harpoon ID 7", silent = true, noremap = true })
    end,
  },
  {
    "stevearc/oil.nvim",
